...
This data package includes the 2013 pressure and final cumulative impacts data.  The pressure data 
are the stressor data after adjusting for the vulnerability of the habitats to a given pressure.  
These pressure data are then summed to calculate the cumulative impacts data.  This is the most complete 
data but is not appropriate for comparing to the 2008 data (for that use data package: Cumulative human impacts: 
2008 and 2013 pressure and cumulative impacts data (subset of pressures)). Pressure data was calculated for each 
stressor by: (1) multiplying the rescaled stressor (rescaled using only the 2013 data) by each habitat layer and 
the corresponding stressor/habitat vulnerability score (for each stressor this generates: 20 rasters); (2) summing 
the resulting stressor/habitat/vulnerability rasters (generates 1 raster for each stressor); (3) dividing by the number 
of habitats found in each raster cell layer.  The cumulative impacts was calculated by summing all the pressure rasters. 
