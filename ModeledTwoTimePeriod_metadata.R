...
This data package includes the 2013 and 2008 pressure and final cumulative impacts data.  
The pressure data are the stressor data after adjusting for the vulnerability of the habitats 
to a given pressure.  These pressure data are then summed to calculate the cumulative impacts data.   
These data do not include pressures: sea level rise, ocean pollution, invasives, and shipping because 
data was not available for 2008.

These files should be used for comparisons of 2008 and 2013 data, but it is important to note that 
they do not include all pressures. Pressure data was calculated for each stressor by: (1) multiplying the 
rescaled stressor (rescaled using the maximum value across both 2008 and 2013 data) by each habitat layer and 
the corresponding stressor/habitat vulnerability score (for each stressor this generates: 20 rasters); 
(2) summing the resulting stressor/habitat/vulnerability rasters (generates 1 raster for each stressor); 
